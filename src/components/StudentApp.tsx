      ...
      <div className="relative h-screen w-full bg-[#020617] overflow-hidden" dir={dir}>
        ...
        {loading && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#0f172a]/80 backdrop-blur-md">
            ...
            <h2 className="text-lg font-black text-white">جاري جلب بياناتك...</h2>
          </div>
        )}
        {loginError && !loading && (
          <div className="absolute top-10 left-4 right-4 z-50 bg-rose-500/10 border border-rose-500/50 backdrop-blur-md p-4 rounded-2xl shadow-lg">
            <p className="text-sm font-black text-rose-400 text-center">{loginError}</p>
            <button onClick={() => setLoginError('')} className="mt-2 w-full bg-rose-500/20 py-2 rounded-xl text-xs font-bold text-white">حسناً</button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden relative text-white bg-[#020617]" dir={dir}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative z-10 pt-safe">
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-[100px] w-full relative z-10">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[9px] font-black ${isActive ? 'text-cyan-100' : 'text-slate-500'}`}>{item.label}</span>
              </button>
            );
          })}
        </div>
