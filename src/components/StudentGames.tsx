...
export interface GameQuestion {
  ...
  archivedAt?: string;
  visibleFrom?: string;
  pairs?: Array<{ term?: string; definition?: string; left?: string; right?: string }>;
  sequence?: string[];
}
...
type ActiveGame = 'snake_ladder' | 'knowledge_race' | 'football_quiz' | 'super_taleb' | 'true_false' | 'match_cards' | 'sequence_order' | null;
...
export interface StudentGameResultLogEntry {
  ...
  parentSyncStatus?: 'pending_sync' | 'synced' | 'failed' | 'skipped';
...
type GameCardWithAvailability = GameCard & { questionCount: number };
...
const BASE_GAMES: Omit<GameCard, 'status'>[] = [
  ...
  { id: 'super_taleb', title: 'سوبر طالب', shortTitle: 'سوبر طالب', description: 'تحرك واقفز، اجمع العملات ونجوم المعرفة، وتجاوز العقبات حتى تصل إلى الفصل الدراسي.', icon: GraduationCap, color: 'success', supportedGameTypes: ['super_taleb', 'platform', 'adventure'], supportedQuestionTypes: ['multiple_choice', 'true_false'], minQuestions: 0, estimatedTime: '4 - 7 دقائق' },
  ...
  { id: 'sequence_order', title: 'رتّب الأحداث', shortTitle: 'الترتيب', description: 'رتب الأحداث أو الخطوات حسب التسلسل الصحيح.', icon: ListOrdered, color: 'primary', supportedGameTypes: ['sequence', 'order'], supportedQuestionTypes: ['sequence'], minQuestions: 1, estimatedTime: '3 دقائق' }
];
...
const getToneClasses = (color: GameCard['color']) => {
  switch (color) {
    case 'warning': return { icon: 'bg-warning/10 text-warning border-warning/20', button: 'bg-warning text-white' };
    case 'success': return { icon: 'bg-success/10 text-success border-success/20', button: 'bg-success text-white' };
    case 'info': return { icon: 'bg-info/10 text-info border-info/20', button: 'bg-info text-white' };
    case 'danger': return { icon: 'bg-danger/10 text-danger border-danger/20', button: 'bg-danger text-white' };
    default: return { icon: 'bg-primary/10 text-primary border-primary/20', button: 'bg-primary text-white' };
  }
};
...
const filterPlayableQuestions = (questions: GameQuestion[]) => questions.filter(question => {
...
const toSharedQuizShape = (questions: GameQuestion[]) => filterPlayableQuestions(questions).map(question => ({
  id: question.id,
...
const toMatchCardsQuestions = (questions: GameQuestion[]): MatchCardsQuestion[] => questions
  .filter(question => {
    if (question.active === false) return false;
...
const toSequenceOrderQuestions = (questions: GameQuestion[]): SequenceOrderQuestion[] => questions
  .filter(question => {
    if (question.active === false) return false;
    return question.questionType === 'sequence' && Array.isArray(question.sequence) && question.sequence.map(item => item.trim()).filter(Boolean).length >= 2;
  })
  .map(question => ({
    id: question.id,
    subject: question.subject,
...
const inferMainQuestionMeta = (questions: GameQuestion[]) => {
  ...
  return { subject: first?.subject || '', unit: first?.unit || '', lesson: first?.lesson || '' };
};
...
const inferTotalQuestions = (result: UnifiedGameResult, questions: GameQuestion[]) => (
  getResultNumber(result, 'totalQuestions') || getResultNumber(result, 'questionsCount') || getResultNumber(result, 'total') || questions.length || 0
);

const inferCorrectAnswers = (result: UnifiedGameResult) => (
  getResultNumber(result, 'correct') || getResultNumber(result, 'correctAnswers') || getResultNumber(result, 'matched') || 0
);
...
const buildWrongDetails = (weakQuestionIds: string[], questions: GameQuestion[]) => {
  ...
  return weakQuestionIds.map(questionId => {
    ...
    return {
      ...
      unit: q?.unit,
      lesson: q?.lesson,
      correctAnswerText: q?.correctAnswerText || (typeof q?.correctAnswerIndex === 'number' ? q?.options?.[q.correctAnswerIndex] : undefined),
...
const getAttemptNumberForLesson = (studentKey: string, gameType: string, subject?: string, lesson?: string) => {
  ...
  const sameContextAttempts = oldLog.filter(item => item.gameType === gameType && (item.subject || '') === (subject || '') && (item.lesson || '') === (lesson || ''));
  return sameContextAttempts.length + 1;
};

const getGameTypeQuestions = (gameType: string, collections: Record<UnifiedGameType, GameQuestion[]>) => {
  if (gameType in collections) return collections[gameType as UnifiedGameType];
  return [];
};

const getCurrentSchoolName = () => (
  localStorage.getItem('rased_student_school_name') ||
  localStorage.getItem('rased_school_name') ||
  localStorage.getItem('rased_parent_school_name') ||
  ''
);
...
const buildGameResultLogEntry = (
  ...
  return {
    ...
    schoolName: cloudMeta.schoolName,
...
const StudentGames: React.FC<StudentGamesProps> = ({ student, onGameActiveChange }) => {
  ...
  const games = useMemo<GameCardWithAvailability[]>(() => BASE_GAMES.map(game => {
    ...
    const status: GameStatus = game.id === 'snake_ladder' || game.id === 'super_taleb'
      ? 'available'
  ...
  const findBaseGame = (id: UnifiedGameType) => BASE_GAMES.find(game => game.id === id);

  const snakeLadderQuestions = useMemo(() => {
    const game = findBaseGame('snake_ladder');
    return game ? toSnakeLadderQuestions(currentGameQuestions.filter(q => isQuestionCompatibleWithGame(q, game))) : [];
  }, [currentGameQuestions]);

  const knowledgeRaceQuestions = useMemo(() => {
    const game = findBaseGame('knowledge_race');
    return game ? toKnowledgeRaceQuestions(currentGameQuestions.filter(q => isQuestionCompatibleWithGame(q, game))) : [];
  }, [currentGameQuestions]);
  ...
  const superTalebQuestions = useMemo(() => {
    const game = findBaseGame('super_taleb');
    const compatible = game ? currentGameQuestions.filter(q => isQuestionCompatibleWithGame(q, game)) : [];
    // سوبر طالب تقبل قائمة فارغة وتستخدم أسئلة مدمجة للمعاينة عند عدم توفر أسئلة المعلم.
  ...
  const matchCardsQuestions = useMemo(() => {
    ...
    return game ? toMatchCardsQuestions(currentGameQuestions.filter(q => isQuestionCompatibleWithGame(q, game))) : [];
  }, [currentGameQuestions]);

  const sequenceOrderQuestions = useMemo(() => {
    const game = findBaseGame('sequence_order');
    return game ? toSequenceOrderQuestions(currentGameQuestions.filter(q => isQuestionCompatibleWithGame(q, game))) : [];
  }, [currentGameQuestions]);

  const questionCollections = useMemo<Record<UnifiedGameType, GameQuestion[]>>(() => ({
    ...
    super_taleb: superTalebQuestions as unknown as GameQuestion[],
  ...
  const getResultCloudMeta = (): ResultCloudMeta => {
    ...
    return {
      schoolCode: firstQuestionWithMeta?.schoolCode || localStorage.getItem('rased_student_school_code') || localStorage.getItem('rased_admin_school_code') || '',
      teacherId: firstQuestionWithMeta?.teacherId || localStorage.getItem('rased_student_teacher_id') || localStorage.getItem('rased_teacher_civil_id') || '',
  ...
  const handleStartGame = (game: GameCardWithAvailability) => {
    ...
    if (game.id === 'super_taleb') { setSelectedGame(null); setActiveGame('super_taleb'); return; }
  ...
  const syncGameResultToCloud = async (logEntry: StudentGameResultLogEntry) => {
    ...
    try {
      ...
      console.error('Failed to sync game result to student cloud', error);
      return false;
    }
  };

  const syncGameResultToParentCloud = async (logEntry: StudentGameResultLogEntry) => {
    const meta = getResultCloudMeta();
    try {
      const response = await fetch(PARENT_APP_URL, {
        method: 'POST',
        body: JSON.stringify({
          action: 'studentGameResult',
          rasedId: logEntry.studentId,
          parentCode: logEntry.studentId,
          civilID: logEntry.studentId,
          studentName: logEntry.studentName || meta.studentName,
          schoolName: logEntry.schoolName || meta.schoolName,
          schoolCode: logEntry.schoolCode || meta.schoolCode,
          className: logEntry.className || meta.className,
          grade: logEntry.grade || meta.grade,
          subject: logEntry.subject || 'غير محدد',
          gameResult: {
            ...logEntry,
            schoolName: logEntry.schoolName || meta.schoolName,
            schoolCode: logEntry.schoolCode || meta.schoolCode,
      ...
      console.warn('تعذر مزامنة نتيجة اللعبة مع راصد ولي الأمر', error);
      return false;
    }
  };

  const updateLocalLogsAfterSync = (logKey: string, latestKey: string, logEntryId: string, patch: Partial<StudentGameResultLogEntry>) => {
    try {
      const list = readJsonArray<StudentGameResultLogEntry>(logKey).map(item => item.id === logEntryId ? { ...item, ...patch } : item);
      ...
      if (latestRaw) {
        ...
        if (latestParsed.id === logEntryId) localStorage.setItem(latestKey, JSON.stringify({ ...latestParsed, ...patch }));
      }
    } catch (error) {
      console.error('Failed to update game result sync flags', error);
    }
  };
  ...
  const handleGameComplete = (result: UnifiedGameResult) => {
    ...
    try {
      ...
      const activeQuestionsForResult = getGameTypeQuestions(resultGameType, questionCollections);
      ...
      if (isReviewMode) {
        ...
        const reviewLogEntry: StudentGameResultLogEntry = { ...logEntry, syncStatus: 'local_only', parentSyncStatus: 'skipped' };
        localStorage.setItem(reviewLogKey, JSON.stringify([reviewLogEntry, ...oldReviewLog].slice(0, 100)));
        return;
      }
      ...
      Promise.all([syncGameResultToCloud(logEntry), syncGameResultToParentCloud(logEntry)]).then(([studentSuccess, parentSuccess]) => {
        if (studentSuccess) {
          const pendingAfterSync = readJsonArray<StudentGameResultLogEntry>(pendingSyncKey).filter(item => item.id !== logEntry.id);
          localStorage.setItem(pendingSyncKey, JSON.stringify(pendingAfterSync));
        }
        updateLocalLogsAfterSync(logKey, latestKey, logEntry.id, {
          syncStatus: studentSuccess ? 'synced' : 'pending_sync',
          parentSyncStatus: parentSuccess ? 'synced' : 'failed'
        });
      });
    } catch (error) {
  ...
  return (
    <div className="rased-student-light flex flex-col h-full min-h-0 bg-bgMain text-textPrimary relative overflow-hidden" dir={dir}>
      <header className="sticky top-0 z-40 bg-bgCard border-b border-borderColor pt-[max(env(safe-area-inset-top),16px)] pb-4 px-5 shrink-0 shadow-sm transition-all">
        ...
        <p className="text-[10px] font-bold text-textSecondary pr-7">{t('studentGamesSubtitle') || 'راجع دروسك من خلال ألعاب قصيرة وممتعة 🎮'}</p>

        <div className="mt-4 bg-bgSoft border border-borderColor rounded-2xl p-1 flex gap-1">
          <button type="button" onClick={() => handleModeChange('daily')} className={`flex-1 h-10 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all ${gamesMode === 'daily' ? 'bg-primary text-white shadow-sm' : 'text-textSecondary'}`}>
            <Sparkles className="w-4 h-4" />
            تحديات اليوم
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${gamesMode === 'daily' ? 'bg-white/20 text-white' : 'bg-bgCard text-textMuted'}`}>{gameQuestions.length}</span>
          </button>
          <button type="button" onClick={() => handleModeChange('review')} className={`flex-1 h-10 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all ${gamesMode === 'review' ? 'bg-primary text-white shadow-sm' : 'text-textSecondary'}`}>
            <Archive className="w-4 h-4" />
            مراجعاتي
            <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${gamesMode === 'review' ? 'bg-white/20 text-white' : 'bg-bgCard text-textMuted'}`}>{reviewGameQuestions.length}</span>
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 min-h-0 overflow-y-auto overscroll-contain custom-scrollbar px-5 pt-5 pb-[calc(env(safe-area-inset-bottom)+180px)] space-y-5">
        ...
        {totalQuestions === 0 && (
          <section className="bg-warning/10 border border-warning/20 rounded-3xl p-4 shadow-sm flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-warning/10 text-warning border border-warning/20 flex items-center justify-center shrink-0"><Info className="w-5 h-5" /></div>
            <div>
              <h3 className="text-sm font-black text-textPrimary mb-1">{isReviewMode ? 'لا توجد مراجعات بعد' : 'بانتظار ألعاب المعلم'}</h3>
              <p className="text-[10px] font-bold text-textSecondary leading-6">{isReviewMode ? 'ستظهر هنا أسئلة الأرشيف عندما ينشر المعلم دفعات جديدة وتنتقل الأسئلة القديمة إلى المراجعات.' : 'ستظهر الألعاب عندما يضيف المعلم أسئلة وأنشطة من راصد المعلم. يمكن إبقاء هذه الصفحة جاهزة كمركز ألعاب ديناميكي.'}</p>
            </div>
          </section>
        )}
      ...
      {selectedGame && (
        <>
          <button type="button" aria-label="إغلاق تفاصيل اللعبة" onClick={() => setSelectedGame(null)} className="fixed inset-0 z-[100] bg-slate-900/20" />
