...
export default function StudentKnowledgeFortressGame({ questions, onComplete, onExit }: Props) {
  ...
  useEffect(() => {
    ...
    onComplete?.({ gameType: 'knowledge_fortress', score, correctAnswers: correct, wrongAnswers: wrong, wavesCompleted: Math.min(wave, 4), fortressHealth: health, playedAt: new Date().toISOString() });
  }, [screen]);
  ...
  return <div dir="rtl" className="fixed inset-0 z-[2147483647] overflow-hidden bg-[#06152e] text-white">
    ...
    {screen === 'result' && <div className="relative h-full flex items-center justify-center p-4">
      <section className="w-full max-w-md rounded-[2.2rem] p-7 text-center bg-gradient-to-b from-[#183b68] to-[#07152f] border border-yellow-300/40 shadow-2xl">
        ...
        <div className="grid grid-cols-2 gap-3"><button onClick={start} className="h-13 rounded-2xl bg-yellow-400 text-slate-950 font-black flex items-center justify-center gap-2"><RotateCcw className="w-5 h-5" />جولة جديدة</button><button onClick={onExit ?? (() => setScreen('menu'))} className="h-13 rounded-2xl border border-white/10 bg-white/5 font-black">العودة</button></div>
      </section>
    </div>}
  </div>;
}
